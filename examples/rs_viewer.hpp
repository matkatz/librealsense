...
private:
    ...
    void render_thread()
    {
        ...
        //_window = std::make_shared<GLFWwindow>();
        //register_glfw_callbacks(*_window.get(), _glfw_state);
    ...
    void update_viewports_grid(rs2::video_frame f)
    {
        auto p = f.get_profile();
        auto it = _frame_size.find({ p.stream_type(), p.stream_index() });
        if (it == _frame_size.end())
            _frame_size[{p.stream_type(), p.stream_index()}] = { (float)f.get_width(), (float)f.get_height() };
