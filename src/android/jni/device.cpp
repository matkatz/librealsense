        ...
        xml_parser_function>& format_type_to_lambda, JNIEnv *env, jlong handle)
{
    ...
    auto result = send_and_receive_raw_data(env, reinterpret_cast<rs2_device *>(handle), raw_data);
    ...
    std::string rv;
    decode_string_from_raw_data(command, cmd_xml.custom_formatters, result.data(), result.size(), rv, format_type_to_lambda);
    return rv;
}

//extern "C"
//JNIEXPORT jstring JNICALL
//Java_com_intel_realsense_librealsense_DebugProtocol_nSendAndReceiveData(JNIEnv *env,
//                                                                        jclass clazz,
//                                                                        jlong handle,
//                                                                        jstring file_path,
//                                                                        jstring command) {
//    const char *xml_full_file_path = env->GetStringUTFChars(file_path, 0);
...
//    if (!sts)
//        throw std::runtime_error("Provided XML not found!");
//
...
//
//    env->ReleaseStringUTFChars(file_path, xml_full_file_path);
//    env->ReleaseStringUTFChars(command, line);
//
//    const char* rvc = "123";//result.c_str();
//    jstring rv = env->NewStringUTF(rvc);
//    return rv;
//}
