...
namespace librealsense
{
    namespace frame_streamer
    {
        ...
        std::shared_ptr<profile> config::resolve(std::shared_ptr<streamer> str, const std::chrono::milliseconds& timeout)
        {
            std::lock_guard<std::mutex> lock(_mtx);
            _resolved_profile.reset();
        ...
        bool config::can_resolve(std::shared_ptr<streamer> str)
        {
            try
